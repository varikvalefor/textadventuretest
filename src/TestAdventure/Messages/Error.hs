...
-- room.
invalidRoom :: String;
invalidRoom = "You attempt to enter the specified room but quickly \
              \realise that the room in question does not actually \
              \exist.\nAfter sitting down and pondering potential \
              \other lies for a few moments, you regain what little \
              \sense you have and are again ready to do stuff.";
...
lrTableDebrisAlreadyJunked = "You already junked the remains of the \
                             \table.\nYou CAN retrieve the table's \
                             \remains if you so desire.";
...
lrDebrisNotBroken = "The table is somehow reduced to a pile of garbage \
                    \without actually being broken.  Determining the \
                    \process which is used to modify the table as \
                    \described is left as an exercise for the reader.";

-- | lrTableNotJunk is output if the player attempts to junk the
-- non-broken living room table.
lrTableNotJunk :: String;
lrTableNotJunk = "Why in God's name would you junk a perfectly good \
                 \-- actually a bit cheesy -- actually very cheesy, \
                 \but still functional -- table?!  The thing still \
                 \at least somewhat works, although slightly nudging \
                 \the table can lead to spilled coffee.";
...
-- possess.
noSuchWeapon :: String;
noSuchWeapon = "You have no such weapon.  Maybe you should have \
               \stayed away from those pills.";
...
multipleSuchWeapons = "tat does not currently support selecting \
                      \weapons which have multiple copies.";
