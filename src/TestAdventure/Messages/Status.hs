...
-- | @lrTableCrush@ is written to the standard output if the
-- flimsy-looking living room table is demolished.
lrTableCrush :: String;
...
-- | @lrTableCrushed@ is written to the standard output if the player
-- attempts to smash the flimsy-looking living room table... after
-- having already smashed the living room table.
lrTableCrushed :: String;
lrTableCrushed = "You already smashed the table some time ago.  Hell, \
                 \the thing is still smoking.\nRegardless of this \
                 \fact, you shout and attempt to smash the deris.  \
                 \Determining whether or not this attempt is \
                 \successful is left as an exercise for the reader.";

-- | @lrTableTotesDestroyed@ is written to the standard output if the
-- player character attempts to destroy the living room table... after
-- having already completely trashed the thing.
lrTableTotesDestroyed :: String;
lrTableTotesDestroyed = "Stop!  Stop!  He's already dead!";

-- | @travelSuccess@ is written to the standard output if the player
-- character successfully travels to a player-specified destination.
travelSuccess :: String;
travelSuccess = "You travel successfully.";

-- | @tableFlippedUD@ is written to the standard output if the player
-- character flips the living room table upside-down.
tableFlippedUD :: String;
tableFlippedUD = "You grab the card table and throw the thing into the \
                 \air.  By sheer coincidence, the table manages to \
                 \land exactly as the table was a few moments ago, \
                 \with the exception of being upside-down.";

-- | @tableFlippedUU@ is written to the standard output if the player
-- character restores the table to the table's original position, i.e.,
-- "de-flips" the table.
tableFlippedUU :: String;
tableFlippedUU = "You restore the table to the table's original \
                 \position, i.e., you \"de-flip\" the table.";
...
-- | @mopSmashed@ is written to the standard output if the player
-- character mutilates the mop.
mopSmashed :: String;
mopSmashed = "You mutilate the mop.";

-- | @mopAlreadySmashed@ is written to the standard output if the player
...
-- | @weaponWielded@ is written to the standard output if the player
-- successfully wields a weapon.
weaponWielded :: String;
weaponWielded = "You weld the weapon to your hand.\n\
                \You do know that you could have just grabbed the \
                \weapon, no?";
